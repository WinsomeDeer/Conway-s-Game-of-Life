        ...
        if self.running:
            for i in range(self.n):
                for j in range(self.n):
                    cell = self.cells[i,j]
                    total = self.a[i%self.n, (j+1)%self.n] + self.a[i%self.n, (j-1)%self.n] + self.a[(i-1)%self.n, j] + self.a[(i+1)%self.n, j] + self.a[(i+1)%self.n, (j+1)%self.n] + self.a[(i-1)%self.n, (j+1)%self.n] + self.a[(i-1)%self.n, (j-1)%self.n] + self.a[(i+1)%self.n, (j-1)%self.n]
                    if self.a[i,j] == 1:
